...
public class MeetingController {
    ...
    public ResponseEntity<MeetingDTO> createMeeting(@Valid @RequestBody MeetingCreateRequest req) {
        ...
        List<String> emails = parts.stream().map(Participant::getEmail).collect(Collectors.toList());
        MeetingDTO dto = new MeetingDTO(m.getId(), m.getCalendarId(), m.getTitle(), m.getOrganizerId(), m.getStartTime(), m.getEndTime(), m.getStatus(), emails);
    ...
    public List<MeetingDTO> listMeetings(@RequestParam UUID calendarId,
        ...
        return meetings.stream().map(m -> {
            List<Participant> parts = participantRepository.findByMeetingId(m.getId());
            List<String> emails = parts.stream().map(Participant::getEmail).collect(Collectors.toList());
            return new MeetingDTO(m.getId(), m.getCalendarId(), m.getTitle(), m.getOrganizerId(), m.getStartTime(), m.getEndTime(), m.getStatus(), emails);
        }).collect(Collectors.toList());
    }

    private MeetingDTO toDto(Meeting m) {
        return new MeetingDTO(m.getId(), m.getCalendarId(), m.getTitle(), m.getOrganizerId(), m.getStartTime(), m.getEndTime(), m.getStatus(), null);
    }
}
